...
class SafeQwen25VLPartialQuantWrapper(SafeQwen25VLWrapper):
    """
    SafeQwen wrapper variant that keeps selected LLM decoder layers in FP16
    while quantizing the remaining modules with bitsandbytes.
    """
    ...
    def __init__(
        ...
        quant_scope: str = "all",
        fp16_llm_layer_start: int = 9,
        fp16_llm_layer_end: int = 17,
    ):
        ...
        self.fp16_llm_layer_start = fp16_llm_layer_start
        self.fp16_llm_layer_end = fp16_llm_layer_end
        ...
        if self.fp16_llm_layer_start < 0 or self.fp16_llm_layer_end < self.fp16_llm_layer_start:
            raise ValueError(
                "Invalid FP16 layer range: ensure start >= 0 and end >= start"
            )
        ...
        if quantization == "bitsandbytes":
            ...
            if self.quant_scope == "vision_only":
                skip_modules.extend(self._build_non_vision_skip_modules())
                print("Using selective quantization: quantizing vision modules only")
            elif self.quant_scope == "llm_only":
                skip_modules.extend(self._build_vision_skip_modules())
                print("Using selective quantization: quantizing LLM modules only")

            # Keep the selected language-model block range in full precision.
            fp16_layer_modules = self._build_fp16_llm_layer_skip_modules(
                self.fp16_llm_layer_start,
                self.fp16_llm_layer_end,
            )
            skip_modules.extend(fp16_layer_modules)

            skip_modules = list(dict.fromkeys(skip_modules))

            print(
                "Keeping LLM layers "
                f"{self.fp16_llm_layer_start}-{self.fp16_llm_layer_end} in FP16 "
                f"({len(fp16_layer_modules)} modules)"
            )
            print(f"Total modules skipped from quantization: {len(skip_modules)}")
    ...
    def _build_fp16_llm_layer_skip_modules(self, start_idx: int, end_idx: int) -> List[str]:
        """Build skip list for model.language_model.layers.<idx> submodules."""
        print("Building FP16 LLM layer skip list (one-time module scan)...")
        ...
        if len(skip_modules) == 0:
            raise RuntimeError(
                "Failed to build FP16 LLM-layer skip list for the requested layer range."
            )
...
def evaluate_safeqwen_resume(args):
    ...
    if args.quantization == "bitsandbytes" and args.quant_scope == "vision_only":
        ...
        suffix += "_llmonly"
    if args.quantization == "bitsandbytes" and args.quant_scope in ["all", "llm_only"]:
        suffix += f"_fp16llm{args.fp16_llm_layer_start}to{args.fp16_llm_layer_end}"
    ...
    if start_idx >= num_samples:
        ...
        model = SafeQwen25VLPartialQuantWrapper(
            ...
            fp16_llm_layer_start=args.fp16_llm_layer_start,
            fp16_llm_layer_end=args.fp16_llm_layer_end,
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SafeQwen2.5-VL Evaluation with Resume (Partial Quantization)")
    ...
    parser.add_argument(
        "--fp16_llm_layer_start",
        type=int,
        default=9,
        help="First language-model decoder layer index to keep in FP16"
    )
    parser.add_argument(
        "--fp16_llm_layer_end",
        type=int,
        default=17,
        help="Last language-model decoder layer index to keep in FP16"
    )
